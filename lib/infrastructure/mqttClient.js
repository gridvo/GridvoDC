...
function MqttClient(mqttBrokerUrl, optionsStr) {
    ...
    this.client.on('connect', function () {
        mc.client.on('message', function (topic, message) {
            ...
            if (topic.indexOf("setStationDVConfig/") == 0) {
                var stationDVConfig = JSON.parse(message.toString());
                mc.emit("StationSetDVConfig", stationDVConfig);
            }
            if (topic.indexOf("stationRDConfig/") == 0) {
                var stationName;
                var stationNameStartIndex = topic.indexOf("/", 0);
                stationName = topic.slice(stationNameStartIndex + 1);
                mc.emit("StationRDConfig", stationName);
            }
            if (topic.indexOf("stationDVConfig/") == 0) {
                var stationName;
                var stationNameStartIndex = topic.indexOf("/", 0);
                stationName = topic.slice(stationNameStartIndex + 1);
                mc.emit("StationDVConfig", stationName);
            }
        });
    });
};
...
MqttClient.prototype.subscribeStationRDConfig = function () {
    this.client.subscribe('stationRDConfig/#');
};
...
MqttClient.prototype.subscribeStationDVConfig = function () {
    this.client.subscribe('stationDVConfig/#');
};

MqttClient.prototype.subscribeStationSetStationDVConfig = function () {
    this.client.subscribe('setStationDVConfig/#');
};
...
MqttClient.prototype.publishStationPubRTData = function (data) {
    this.client.publish(`${data.stationName}/pubRTData`, JSON.stringify(data));
};

MqttClient.prototype.publishStationRDConfig = function (data) {
    this.client.publish(`${data.stationName}/stationRDConfig`, JSON.stringify(data));
};

MqttClient.prototype.publishStationDVConfig = function (data) {
    this.client.publish(`${data.stationName}/stationDVConfig`, JSON.stringify(data));
};
