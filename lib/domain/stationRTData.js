    ...
    if (dataStation.rTDataConfigs) {
        for (let dataName of _.keys(dataStation.rTDataConfigs)) {
            if (!_.isUndefined(dataStation.rTDataConfigs[dataName].openRDM) && dataStation.rTDataConfigs[dataName].openRDM) {
                ...
                rTData.pubDataSpace = rTdataConfig.pubDataSpace ? rTdataConfig.pubDataSpace : rTData.timeSpace;
                let currentDate = new Date();
                rTData.lastPubDataTimestamp = new Date(`${currentDate.getFullYear()}-${currentDate.getMonth() + 1}-${currentDate.getDate()} ${currentDate.getHours()}:00:00`);
...
StationRTData.prototype.addRTData = function (dataName, data) {
    ...
    if ((data.timestamp.getTime() - this.rTDatas[dataName].lastDataTimestamp.getTime()) / this.rTDatas[dataName].timeSpace < 1) {
        return;
    }
